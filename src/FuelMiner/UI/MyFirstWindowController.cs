...
using FuelMiner.Unity.Runtime;
using UitkForKsp2.API;
using UnityEngine;
using UnityEngine.UIElements;

namespace FuelMiner.UI;
...
public class MyFirstWindowController : MonoBehaviour
{
    ...
    public bool IsWindowOpen
    {
        ...
        set
        {
            ...
            GameObject.Find(FuelMinerPlugin.ToolbarFlightButtonID)
                ?.GetComponent<UIValue_WriteBool_Toggle>()
                ?.SetValue(value);

            // Update the OAB AppBar button state
            GameObject.Find(FuelMinerPlugin.ToolbarOabButtonID)
                ?.GetComponent<UIValue_WriteBool_Toggle>()
                ?.SetValue(value);
        }
    }
